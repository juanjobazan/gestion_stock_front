...
const CreateProductPage = () => {
    ...
    const sendForm = async (values) => {
        try {
            const res = await clienteAxios.post('/product', values, config)
            if (res.status === 200) {
                Swal.fire(
                  res.data.msg,
                  '',
    ...
    return (
        <>
            <Formik
                initialValues={{ codigo: '', descripcion: '', precio: '', stock: '' }}
                            ...
                            <Card style={{ width: '25rem' }} className='card-body'>
                                <Card.Body>
                                    <Form>
                                        ...
                                        <Form.Label className='letra'>Stock</Form.Label>
                                        <Form.Group className="mb-3" controlId="formBasicPassword">
                                            <div className="input-group form-group">
                                                <div className="input-group-prepend">
                                                ...
                                                <Form.Control type="number" name='stock' placeholder='Ingrese la stock' value={values.stock} className={errors.stock && touched.stock && errors.stock && 'is-invalid'} onChange={handleChange} maxLength={18} />
                                            </div>
                                            <small className='text-danger'>{errors.stock && touched.stock && errors.stock}</small>
                                        </Form.Group>
